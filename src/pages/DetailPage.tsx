...
const PokemonDetails: React.FC<{ pokemon: any }> = ({ pokemon }) => {
    if (!pokemon) {
        return null; // or render a loading indicator
    }
    const { types, sprites, name, height, weight, base_experience, order, abilities, moves, stats } = pokemon;

    return (
        <div className={`flex flex-col items-center ${getPokemonBackgroundColor(types[0].type.name)} rounded-lg shadow-md h-screen w-screen pd-4`}>
            <img src={sprites.other['official-artwork'].front_default} alt={name} width={475} height={475} className="mb-2" />
            <h2 className="text-2xl font-bold m-4">Name: {name}</h2>
            <div className="flex flex-row space-x-4">
                <div className={`bg-gray-100 ${pokemonInfoBox}`}>
                    <p>Type: {types[0].type.name}</p>
                </div>
                <div className={`bg-black ${pokemonInfoBox}`}>
                    <p className="text-white">Height: {height / 10} m</p>
                </div>
                <div className={`bg-gray-100 ${pokemonInfoBox}`}>
                    <p>Weight: {weight / 10} kg</p>
                </div>
                <div className={`bg-black ${pokemonInfoBox}`}>
                    <p className="text-white">Base experience: {base_experience} </p>
                </div>
                <div className={`bg-gray-100 ${pokemonInfoBox}`}>
                    <p>Order: {order} </p>
                </div>
...
const PokemonAbility: React.FC<{ abilities: any[] }> = ({ abilities }) => (
    <div className={`bg-black ${pokemonInfoBox}`}>
...
const PokemonMove: React.FC<{ moves: any[] }> = ({ moves }) => (
    <div className={`bg-gray-100 ${pokemonInfoBox}`}>
...
const PokemonStat: React.FC<{ stats: any[] }> = ({ stats }) => (
    <div className={`bg-black ${pokemonInfoBox}`}>
